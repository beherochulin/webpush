...
class VapidKeysGenerateCommand extends Command {
    ...
    public function handle() {
        ...
        if ( ! $this->setKeysInEnvironmentFile($keys) ) return;
    ...
    protected function writeNewEnvironmentFileWith($keys) {
        ...
        if ( ! Str::contains($contents, 'VAPID_PUBLIC_KEY') ) $contents .= PHP_EOL.'VAPID_PUBLIC_KEY=';
        if ( ! Str::contains($contents, 'VAPID_PRIVATE_KEY') ) $contents .= PHP_EOL.'VAPID_PRIVATE_KEY=';
