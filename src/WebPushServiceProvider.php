...
class WebPushServiceProvider extends ServiceProvider {
    ...
    public function boot() {
        ...
        if ( $this->app->runningInConsole() ) $this->definePublishing();
    }
    protected function webPushConfig() {
        ...
        if ( ! empty($webpush['gcm']['key']) ) $config['GCM'] = $webpush['gcm']['key'];
        if ( empty($publicKey) || empty($privateKey) ) return $config;

        $config['VAPID'] = compact('publicKey', 'privateKey');
        $config['VAPID']['subject'] = $webpush['vapid']['subject'];

        if ( empty($config['VAPID']['subject']) ) $config['VAPID']['subject'] = url('/');
